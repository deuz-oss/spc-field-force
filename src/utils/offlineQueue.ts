 ...
 * visit check-in/out — the actions with a clear, idempotent server insert/update
 * and an existing optimistic-local-state pattern. Each op carries the full row
 * it needs (not just an id) because after a cold app restart, hydrateAll() only
 * pulls what the server already has — a still-queued row won't be there yet.
 */
...
const QUEUE_KEY = 'spc_offline_queue_v1';
