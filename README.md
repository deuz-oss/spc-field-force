...
- **Asimetri clockOut/finishVisit saat *online* tapi request-nya sendiri gagal** (bukan kasus offline — itu sudah ditangani antrian di atas): jalur ini masih cuma `console.warn`, tidak rollback dan tidak memberi tahu user, beda dengan `clockIn`/`startVisit` yang sudah rollback+dialog sejak awal. Artinya kalau clock-out/check-out gagal tersimpan karena request itu sendiri ditolak server (bukan karena device offline), agent bisa mengira sudah clock-out padahal masih aktif di server. Belum diperbaiki — kandidat kerja selanjutnya.
