...
- **Offline write queue** — app sekarang butuh koneksi internet untuk clock-in/out, visit, dan perubahan merchant; koneksi putus saat itu = tulisan itu hilang begitu saja (tidak ada retry/antrian). Ini konsekuensi dari pindah ke Supabase (dulu app ini offline-first via AsyncStorage). `clockIn`/`startVisit` sudah di-rollback + menampilkan dialog error kalau gagal tersimpan; tulisan lain (mis. update field visit per-keystroke, update merchant) masih gagal diam-diam (`console.warn` saja) — belum konsisten.
