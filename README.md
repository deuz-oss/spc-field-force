...
- **Offline write queue** — app sekarang butuh koneksi internet untuk clock-in/out, visit, dan perubahan merchant; koneksi putus saat itu = tulisan itu hilang begitu saja (tidak ada retry/antrian). Ini konsekuensi dari pindah ke Supabase (dulu app ini offline-first via AsyncStorage). `clockIn`/`startVisit`/`upsertMerchant` di-rollback + menampilkan dialog error kalau gagal tersimpan; update field visit per-keystroke menampilkan dialog peringatan (tanpa rollback teks yang sudah diketik). Aksi tulis lain (mis. `assignMerchants`, manajemen user) masih `console.warn` saja.
